...
Optionally, the output will be a set of files required to run the simulation in a multiscale mode through the  [Vivarium](https://github.com/vivarium-collective/process-bigraph) framework. In this case each unit will be simulated independently in a sequence of short time intervals, interspaced by communication between units, coordinated by the *Vivarium* multiscale simulator.
